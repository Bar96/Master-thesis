...
import tables       #NB added this line
...
def network_threshold(net,sparsity):
    ...
    #     net[i,i] = m
    s = abs(net.flatten())
    init = (len(s)-np.count_nonzero(net))/float(len(s))  #init = actual percentage of zero element
    diff = sparsity - init      
    num = int(diff * len(s))   #num of element that I have to set to 0
    if num > 0:
        s = s[s > 0]   #pick only the element that are > 0
        s.sort()
        threshold = s[num]    #select the num-th element as threshold
...
def import_csv(data_file,gene_file,cell_file='',sep='\t'):
    ...
    genes = pd.read_csv(os.path.join(gene_file,'genes.tsv'), header=None, sep=sep)   #NB correction: gene_file instead of path
    return X,genes
...
def import_network_mtx(network_file,genename_file,sparsity):
    ...
    return network,np.asarray(names)   #NB correction: names instead of network_genes 

def import_npy(filename,gene_names,network_file,genename_file,sparsity):
    X = np.load(filename)
    genes = np.load(gene_names)
    network,network_genes = import_network(network_file,genename_file,sparsity)  #NB correction: import_network rather than import_network_npy
...
def create_gene_list(gene_pairs):
    gene_list = []
    
    for genes in gene_pairs:
        if not is_present(genes[0], gene_list):
            gene_list.append(genes[0])
        if not is_present(genes[1], gene_list):
            gene_list.append(genes[1])
    return gene_list        
...
def import_network_from_gene_pairs(filename):
    ...
    genes = create_gene_list(genepairs)                               #added this line
    ...
    return network                                 #NB correction: network instead of net
