...
splatSimulate <- function(params = newSplatParams(),
    ...
    if (method == "single") {
        ...
        sim <- splatSimGroupDE(sim, params, network)
...
splatSimGroupDE <- function(sim, params, network) {
    ...
    for (idx in seq_len(nGroups)) {
    	#de.prob = prob of a gene to be selected as DE
    	#de.facLoc = mean of the lognorm distribution to generate the DE factors
    	#de.facScale = variance of the lognorm distribution to generate DE factors
        #de.facs <- getLNormFactors(nGenes, de.prob[idx], de.downProb[idx],
                                   #de.facLoc[idx], de.facScale[idx])
        #modified part
        de.facs <- getLNormFactors2(network, de.downProb[idx],
                                   de.facLoc[idx], de.facScale[idx])                           
...
# added function
getLNormFactors2 <- function(network, neg.prob, fac.loc, fac.scale) {
  
  max_elem <- round((ncol(network)*0.1), 0)
  rnd_indices <- sample(1:ncol(network), 5, replace=F)
  
  counts <- integer(5)
  masks <- matrix(0, 5, max_elem)
  index_mask <- matrix(0, 5, ncol(network))
  
  num_selected_elem <- 0
  for (i in 1:5) {
    if(num_selected_elem == max_elem) break
    network[rnd_indices[i], rnd_indices[i]] <- 1
    row_elem <- sum(network[rnd_indices[i], ])
    if((num_selected_elem + row_elem) > max_elem) {
      counts[i] <- max_elem - num_selected_elem
      masks[i, (num_selected_elem+1):max_elem] <- rep(T, counts[i])
      count <- 0
      j <- 1
      while(count < max_elem - num_selected_elem) {
        index_mask[i, j] <- network[rnd_indices[i], j]
        if(network[rnd_indices[i], j] > 0) count <- count + 1
        j <- j + 1
      }
      num_selected_elem <- max_elem
      
    }
    else {
      counts[i] <- row_elem
      masks[i, (num_selected_elem+1):(num_selected_elem+row_elem)] <- rep(T, row_elem)
      index_mask[i, ] <- network[rnd_indices[i], ]
      num_selected_elem <- num_selected_elem + row_elem
    }
  }
  
  facs_selected = rlnorm(num_selected_elem, fac.loc, fac.scale)
  rnd_dir <- (-1) ^ rbinom(5, 1, neg.prob)
  
  dir_selected <- rep(0, max_elem)
  for(i in 1:5) {
    dir_selected[as.logical(masks[i, ])] <- rep(rnd_dir[i], counts[i])
  }
  dir_selected <- dir_selected[1:num_selected_elem]
  
  dir_selected[facs_selected < 1] <- -1 * dir_selected[facs_selected < 1]
  factors <- facs_selected ^ dir_selected
  
  selected_factors <- rep(1, ncol(network))
  for(i in 1:5) {
    selected_factors[as.logical(index_mask[i, ])] <- factors[as.logical(masks[i, ])]
  }
  print("DE factors generated") 
